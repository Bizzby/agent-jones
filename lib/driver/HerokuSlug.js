...
class HerokuSlug extends EventEmitter {
  ...
  // You probably want use stop or softStop
  kill (callback) {
    ...
    // TODO what happens if get here and the kids are already
    // zombies for some reason?
    // maybe we should keep a map of the process tree
    treeKill(this._slugRunnerProcess.pid, signame, (err) => {
      if (err) {
        _log(
          `Kill process tree of ${this._slugRunnerProcess.pid} with ${signame} failed: ${err.message}`
        )
      } else {
        _log(`Kill process tree of ${this._slugRunnerProcess.pid} with ${signame} finished`)
      }
    })
    // TODO: will this fire if the PID is already dead??
