...
class HerokuSlug extends EventEmitter {
  constructor (workspace, opts) {
    ...
    // FIXME: Stupid lock - set this to a string and check for it when
    // wanting to transition state
  ...
  _kill (signal, cb) {
    ...
    treeKill(this._slugRunnerProcess.pid, signal, (err) => {
      if (err) {
        _log(
          `Kill process tree of ${this._slugRunnerProcess.pid} with ${signal} failed: ${err.message}`
        )
      } else {
        _log(`Kill process tree of ${this._slugRunnerProcess.pid} with ${signal} finished`)
      }
      cb(err)
    })
  }
  ...
  stop (callback) {
    ...
    //FIXME: this does nothing if there is no user supplied callback
    if (this._stateLock) {
        debug('error: statelock %s in place', this._stateLock)
        return callback(new Error('Cannot transition state, existing statelock: ${this._stateLock}'))
    }
